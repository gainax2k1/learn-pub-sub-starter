	...
	for {
		...
		case "spam":
			...
			if len(userInput) > 1 {
				...
				if err != nil {
					fmt.Printf("Unable to spam: %v", err)
					break
				}

				for i := 0; i < n; i++ {
					malMsg := gamelogic.GetMaliciousLog()
					err := PublishGameLog(publishCh, gs.GetUsername(), malMsg)
					if err != nil {
						fmt.Printf("unable to publish spam: %v", err)
						break
					}
...
func handlerWar(gs *gamelogic.GameState, ch *amqp.Channel) func(gamelogic.RecognitionOfWar) pubsub.AckType {
	return func(dw gamelogic.RecognitionOfWar) pubsub.AckType {
		...
		dwOutcome, winner, loser := gs.HandleWar(dw)
...
func PublishGameLog(ch *amqp.Channel, usr string, msg string) error {
	log := routing.GameLog{
